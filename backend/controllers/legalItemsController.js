...
const getAllItems = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM LegalItems');
    res.json(rows);
  } catch (error) {
...
const getItemById = async (req, res) => {
  const { id } = req.params;
  try {
    const [rows] = await pool.query('SELECT * FROM LegalItems WHERE id = ?', [id]);
    if (rows.length > 0) {
      res.json(rows[0]);
    } else {
...
const createItem = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO LegalItems (title, description, category, badges, icon) VALUES (?, ?, ?, ?, ?)',
      [title, description, category, JSON.stringify(badges), icon]
    );
    res.status(201).json({ id: result[0].insertId, title, description, category, badges, icon });
  } catch (error) {
...
const updateItem = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE LegalItems SET title = ?, description = ?, category = ?, badges = ?, icon = ? WHERE id = ?',
...
const deleteItem = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query('DELETE FROM LegalItems WHERE id = ?', [id]);
    if (result[0].affectedRows > 0) {
